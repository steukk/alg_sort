...
- Инициализировать low = 0 и high = len(arr) - 1.
- Выполнять поиск, пока low не превысит high.
- В каждой итерации вычислять середину mid.
- Сравнивать элемент arr[mid] с target.
- Обновлять границы поиска по результату сравнения.
- Если элемент найден — вернуть индекс.
- Если нет — вернуть -1.
...
- Инициализировать границы поиска: low = 0, high = размер массива - 1.
- Пока low <= high и искомое значение x лежит в диапазоне arr[low] и arr[high]:
- Если low == high и элемент равен x, вернуть low, иначе вернуть -1.
